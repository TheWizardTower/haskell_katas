...
main = hspec $ do
    describe "Extracting portion of list" $ do
        ...
        it "can take with a while" $ do
            ...
            takeWhile (\x -> x < 8) (enumFromTo 5 15) `shouldBe` [5,6,7]
            takeWhile (\x -> x == 'a') "abracadabra" `shouldBe` "a"
        it "can drop while" $ do
            dropWhile (\x -> x < 5) [1..10] `shouldBe` [5,6,7,8,9,10]
            dropWhile (\x -> x == 'a') "abracadabra" `shouldBe` "bracadabra"
